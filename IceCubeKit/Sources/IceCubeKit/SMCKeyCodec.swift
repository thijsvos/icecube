...
/// The SMC data types Ice Cube understands, named **exactly** as the SMC reports
...
/// throws `IceCubeError.smcEncodingFailed` instead of being silently adjusted.
///
...
public enum SMCKeyCodec {
    ...
    ///
    /// - Throws: `IceCubeError.smcDecodingFailed` unless `key` is exactly four
    ...
    public static func keyString(from code: UInt32) throws(IceCubeError) -> String {
        ...
        guard bytes.allSatisfy({ (0x20 ... 0x7E).contains($0) }) else {
            ...
            throw IceCubeError.smcDecodingFailed(key: hex, type: "FourCharCode", bytes: bytes)
        }
    ...
    /// - Throws: `IceCubeError.smcDecodingFailed` (carrying the offending
    ...
    /// minimal — the name is the only field Ice Cube needs.
    ///
    ...
    public static func decodeString(_ bytes: [UInt8], forKey key: String = "") throws(IceCubeError) -> String {
        ...
        guard nameField.allSatisfy({ (0x20 ... 0x7E).contains($0) }) else {
            throw IceCubeError.smcDecodingFailed(key: key, type: SMCDataType.fanDescriptor.name, bytes: bytes)
        }
    ...
    ///
    /// - Throws: `IceCubeError.smcEncodingFailed` for NaN/±infinity, values
