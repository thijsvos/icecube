...
- Watchdog + auto-revert everywhere (see §4.3). Quitting the app (by default) returns fans to system auto.
...
- [x] Presets: built-ins (macOS/Quiet/Balanced/Cold/Max — the hand-back preset is named "macOS", not "Auto", because in a fan-control app "Auto" reads as "the app manages it" when it means the opposite) + user presets, JSON in `~/Library/Application Support/IceCube/`, quick-switch in popover.
