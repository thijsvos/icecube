# XCODE_GUIDE.md — The human's handbook for Zephyr
...
**Daily TL;DR**
- Build/run UI work: no Xcode needed — `ZEPHYR_SIMULATED=1`, commands in CLAUDE.md.
- Anything touching the real helper: open Xcode, run the **Zephyr** scheme, and if the helper misbehaves after a rebuild → Debug menu → **Re-register helper**.
...
6. Bundle IDs are already the real ones (`io.github.thijsvos.zephyr` and friends) — there is nothing to rename.
7. Sanity-check (look, don't touch): select the **Zephyr** target → **Signing & Capabilities** — "Automatically manage signing" ✅, your Team shown, and **no App Sandbox capability** (sandboxed apps can't open the SMC or install daemons; Hardened Runtime ON is fine and required for release). Same check for **ZephyrHelper**.
8. Product → Build (⌘B). First build resolves the local ZephyrKit package; be patient.

## 3. Running in simulated mode (no root, no approvals)
Scheme selector (top bar) → **Zephyr (Simulated)** → ⌘R. That's it — the scheme ships pre-generated from `project.yml` with `ZEPHYR_SIMULATED=1` already baked in, so there is nothing to edit. You should see the menu bar item with fake-but-lively data. This is the mode for all UI/chart/curve work.
...
1. **Run the app from /Applications for helper testing.** Product → Build, then drag the built app there (Product → Show Build Folder in Finder → Products/Debug), or use the `scripts/install-debug.sh` Claude Code provides. Registering daemons from Xcode's DerivedData path is flaky and every rebuild changes the binary anyway.
2. Launch Zephyr. Heads-up: it's an **LSUIElement** app — **no Dock icon, ever**; the menu bar item is the only UI, so look up, not down. Go through onboarding → **Enable fan control**. macOS shows a notification / takes you to **System Settings → General → Login Items & Extensions** (older: "Login Items"). Toggle **Zephyr** ON under "Allow in the Background"; enter your admin password if asked.
3. Back in Zephyr, helper status should read *enabled*, and the Debug menu → Helper Status shows a version. Try a manual fan slider — you should hear it.
4. **After every rebuild of the helper**, macOS may still run the *old* registered copy. Fix: Zephyr Debug menu → **Re-register helper** (it unregisters + registers; once approved, re-registering doesn't re-prompt for your password). This is the #1 "why isn't my change live" trap.
...
- **Helper (a root daemon — different rules):**
  - Logs first: `log stream --predicate 'subsystem == "io.github.thijsvos.zephyr"' --level debug` (or Console.app, search the subsystem). Every SMC write is logged with key/value/reason — read this before anything else.
  - Attach a debugger: Xcode → Debug → Attach to Process by PID or Name… → `ZephyrHelper` (Xcode will ask to take you root). Or set the *helper* scheme's Run → "Wait for the executable to be launched", then trigger it from the app.
  - Is it even running? `sudo launchctl print system/io.github.thijsvos.zephyr.helper`.
- **Inspect Background Task Management** (what SMAppService talks to): `sfltool dumpbtm | less` — search for zephyr. Nuclear reset: `sudo sfltool resetbtm` (⚠️ resets approvals for *every* app on the Mac; you'll re-approve things — last resort only).
- **Fans stuck after a crash?** They auto-revert (watchdog), but belt-and-braces: relaunch Zephyr → preset **Auto**, or reboot — SMC returns to system control.
...
| "Zephyr is damaged and can't be opened" on another Mac | Unsigned/un-notarized build + quarantine → recipient runs `xattr -dr com.apple.quarantine /Applications/Zephyr.app` (dev builds only) — or do §8 properly |
| Build suddenly weird after big changes | Clean build folder (⇧⌘K), delete `~/Library/Developer/Xcode/DerivedData/Zephyr-*`, retry |
| Notifications never appear | macOS denied permission → System Settings → Notifications → Zephyr → Allow |
...
2. Store notarization credentials for `notarytool` — pick **one** of the two auth methods; both end in the same keychain profile, so the per-release commands never change.
   ...
   **Option A — App Store Connect API key** (recommended: scoped, revocable, no password to rotate). App Store Connect → Users & Access → **Integrations → App Store Connect API** → create a key (role: Developer), download the `.p8`:
   ```bash
   xcrun notarytool store-credentials "zephyr-notary" \
...
Per release (scripted in `scripts/release.sh` + CI, but know the moves):
   ```bash
   xcodebuild -project Zephyr.xcodeproj -scheme Zephyr -configuration Release archive -archivePath build/Zephyr.xcarchive
   # export with Developer ID (Xcode Organizer → Distribute App → Direct Distribution, or exportOptions.plist in CI)
   ditto -c -k --keepParent build/export/Zephyr.app build/Zephyr.zip
   xcrun notarytool submit build/Zephyr.zip --keychain-profile zephyr-notary --wait
   xcrun stapler staple build/export/Zephyr.app
